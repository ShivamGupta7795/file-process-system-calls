...
sys_write(int fd, const void *buf, size_t buflen, int *err1)
{
	...
	ku.uio_iov = &iov;
	ku.uio_iovcnt = 1;
	ku.uio_offset = proc->file_table[fd]->offset;
	ku.uio_resid = buflen;
...
sys_open(const char *filename, int flags,int *err1)
{
	...
	lock_acquire(proc->file_table[i]->sys_lock);
	proc_fd=i;
	...
	if (result) {
		*err1 = EINVAL;
                       // vfs_close(vn);
                       // vfs_remove(name);
		lock_release(proc->file_table[i]->sys_lock);
		kfree(buf);
		return -1;
	}
	kfree(buf);
...
/*if(result1){
	...
	ui.uio_iov = &iov;
	ui.uio_iovcnt = 1;
	ui.uio_offset = proc->file_table[fd]->offset;
	ui.uio_resid =  buflen;
...
//code for sharing the file handle
	...
	for(int i=0;i<OPEN_MAX;i++)
	{
		...
		//	kprintf("%d--->",curproc->file_table[i]->ref_count);
			//kprintf("inside m: %d\n",i);
			lock_acquire(curproc->file_table[i]->sys_lock);
			curproc->file_table[i]->ref_count++;
			cproc->file_table[i] = proc->file_table[i];
			lock_release(curproc->file_table[i]->sys_lock);
...
//	kprintf("enter waitpid\n");
	...
	if(status==NULL)
	{
		kprintf("%d", cproc->p_numthreads);
		while(1){
			if(cproc->p_numthreads ==1){
				break;
			}
		}
		proc_destroy(cproc);
